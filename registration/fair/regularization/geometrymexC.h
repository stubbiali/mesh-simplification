...
inline void d2SarTriangle3D(double *y1, double *y2, double *yM, double *x1, double *x2, double *xM,const double ARef, double *r1, double *r2, double *rM, double alphaArea){
    double vy[3], wy[3], wx[3], vx[3],cprod[3];
    ...
    double d2A =  x/(ARef*ARef) * alphaArea;
   	double dwA, dvA;
	
	// first component
	dvA =  2.0 * (cprod[1] * wy[2] + cprod[2] * wy[1]); // d_{v[0]} A
	dwA = -2.0 * (cprod[1] * vy[2] + cprod[2] * vy[1]); // d_{w[0]} A
	...
	// second component 
	dvA = 2.0 * (cprod[0] * wy[2] - cprod[2] * wy[0]); // d_{v[1]} A
	dwA = 2.0 * (-cprod[0] *vy[2] + cprod[2] * vy[0]); //d_{w[1]} A
	...
	// third component 
	dvA = 2.0 * (-cprod[0] * wy[1] - cprod[1] * wy[0]); // d_{v[2]} A
	dwA = 2.0 * (cprod[0] *  vy[1]  + cprod[1] * vy[0]); // d_{w[2]} A
	rM[2] += d2A * (-dvA - dwA);
	r1[2] += d2A * ( dvA      );
	r2[2] += d2A * (     + dwA);
}
...
inline void d2SdiagTetra(double *y1, double *y2, double *yF, double *yM,const double ARef,
						 ...
						 double alphaVolume){
    double dVx, vol,vol2, d2S, cof[9],x;
	double vy[3], wy[3];
    int i, j,k;
    ...
    if (alphaArea>0) {
		...
		*/ 
		...
		r1[0] += alphaArea * dAx * (1/(ARef*ARef)) * dAx;
		...
		r1[2] += alphaArea * dAx * (1/(ARef*ARef)) * dAx;

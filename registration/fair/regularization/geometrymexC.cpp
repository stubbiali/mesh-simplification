...
void mexFunction(int nlhs, mxArray *plhs[ ], int nrhs, const mxArray *prhs[ ]) {
    ...
    if (strcmp("V", flag)==0) {
        ...
        if (dim==2) {volume2D(result, yc, m, n);}
        else {volume3D(result, yc, m, n);}
		/* ---------------------------------------------------------------
		 *   CASE "VRange" : Compute range of Vol
		 ---------------------------------------------------------------*/
			...
			result = mxGetPr(plhs[0]);
			if (dim==2) {volumeRange2D(result, yc, m, n);}
			else {volumeRange3D(result, yc, m, n);}
			/* ---------------------------------------------------------------
		 *   CASE "A" : Compute area
		 ---------------------------------------------------------------*/
        ...
        if (dim==2) {Jac2D(result, yc, m, n);}
        else {Jac3D(result, yc, m, n);}
		/* ---------------------------------------------------------------
        ...
        result  = mxGetPr(plhs[0]);
        for (i=0; i<n; i++) {result[i]=0.0;}
        if (dim==2) {dJacx2D(result, yc, x, m, n);}
        else{dJacx3D(result, yc, x, m, n);}
		/* ---------------------------------------------------------------
        ...
        if (dim==2) {dJacTx2D(result, yc, x, m, n);}
        else{dJacTx3D(result, yc, x, m, n);}
		/* ---------------------------------------------------------------
		 *   CASE "S" : Compute S
		 ---------------------------------------------------------------*/
        ...
        h     =  mxGetPr(prhs[6]);
		doDerivative = (bool) mxGetLogicals(prhs[7]);
	   ...
	   if (dim==2) {
		...
		/* ---------------------------------------------------------------
		 *   CASE "EPIdiag" : Compute diag of d2J for EPI
		 ---------------------------------------------------------------*/
	}  else if (strcmp("EPIdiag", flag)==0) {
		//
		// Usage: diag = geometrymexC(yc,m,'EPIdiag',yc2,Jac1,Jac2,dI1,dI2,I1,I2,v1,v2,h)
		//
		// get yc2
		mrows = mxGetM(prhs[3]);
		ncols = mxGetN(prhs[3]);
		if (!mxIsDouble(prhs[3]) || !(mrows==nn*dim && ncols==1))
			mexErrMsgTxt("yc2 has wrong size.");
		yc2     =  mxGetPr(prhs[3]);
		
		// get Jac1
		mrows = mxGetM(prhs[4]);
		ncols = mxGetN(prhs[4]);
		if (!mxIsDouble(prhs[4]) || !(mrows==n && ncols==1))
			mexErrMsgTxt("Jac1 has wrong size.");
		Jac1     =  mxGetPr(prhs[4]);
		// get Jac2
		mrows = mxGetM(prhs[5]);
		ncols = mxGetN(prhs[5]);
		if (!mxIsDouble(prhs[5]) || !(mrows==n && ncols==1))
			mexErrMsgTxt("Jac2 has wrong size.");
		Jac2     =  mxGetPr(prhs[5]);

		// get dI1
		mrows = mxGetM(prhs[6]);
		ncols = mxGetN(prhs[6]);
		if (!mxIsDouble(prhs[6]) || !(mrows==n*dim && ncols==1))
			mexErrMsgTxt("dI1 has wrong size.");
		dI1     =  mxGetPr(prhs[6]);
		// get dI2
		mrows = mxGetM(prhs[7]);
		ncols = mxGetN(prhs[7]);
		if (!mxIsDouble(prhs[7]) || !(mrows==n*dim && ncols==1))
			mexErrMsgTxt("dI2 has wrong size.");
		dI2     =  mxGetPr(prhs[7]);
		
		// get I1
		mrows = mxGetM(prhs[8]);
        ncols = mxGetN(prhs[8]);
     	if (!mxIsDouble(prhs[8]) || !(mrows==n && ncols==1))
            mexErrMsgTxt("I1 has wrong size.");
        I1     =  mxGetPr(prhs[8]);
		// get I2
		mrows = mxGetM(prhs[9]);
        ncols = mxGetN(prhs[9]);
     	if (!mxIsDouble(prhs[9]) || !(mrows==n && ncols==1))
            mexErrMsgTxt("I2 has wrong size.");
        I2     =  mxGetPr(prhs[9]);
		
		// get v1
		mrows = mxGetM(prhs[10]);
        ncols = mxGetN(prhs[10]);
     	if (!mxIsDouble(prhs[10]) || !(mrows==dim && ncols==1))
            mexErrMsgTxt("v1 has wrong size.");
        v1     =  mxGetPr(prhs[10]);
		// get v2
		mrows = mxGetM(prhs[11]);
        ncols = mxGetN(prhs[11]);
     	if (!mxIsDouble(prhs[11]) || !(mrows==dim && ncols==1))
            mexErrMsgTxt("v2 has wrong size.");
        v2     =  mxGetPr(prhs[11]);
		
		mrows = mxGetM(prhs[12]);
        ncols = mxGetN(prhs[12]);
     	if (!mxIsDouble(prhs[12]) || !(mrows==1 && ncols==dim))
            mexErrMsgTxt("h has wrong size.");
        h     =  mxGetPr(prhs[12]);
		...
		for (i=0; i<nn; i++) {result[i]=0.0;}
		if (dim==2) {
			EPIdiag2D(result, yc,yc2,m,Jac1,Jac2,dI1,dI2,I1,I2,v1,v2,h);
		}
        else{
			EPIdiag3D(result, yc,yc2,m,Jac1,Jac2,dI1,dI2,I1,I2,v1,v2,h);
		}
    }else {

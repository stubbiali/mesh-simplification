...
/*! Classe che implementa l'algoritmo per diffondere la "malattia" in una mesh2d. È stato deciso di rendere questa funzionalità
una classe in modo tale da poter rendere più flessibile il suo utilizzo*/
    
template<typename GEOSHAPE, UInt DIM=3> class virus2d : public connect2d<GEOSHAPE>
{  
	...
	public:
		...
		virus2d(mesh2d<GEOSHAPE>  * _meshPointer);
	...
	public:
		...
		/*! Metodo che diffonde la malattia partendo da un elemento
		    ...
		    \param idStart identificatore dell'elemento da cui partire */
		void virusDiffusion(mesh1d<BSHAPE> * barrier, UInt idStart=0);
		...
		void createSet(mesh1d<BSHAPE> * barrier, set<geoElement<BSHAPE> > * finder);
...
// Costruttori
//
template<typename GEOSHAPE, UInt DIM> virus2d<GEOSHAPE, DIM>::virus2d()
{
	typename connect2d<GEOSHAPE>::connect2d();
	
	// setto la tolleranza
	toll = 1e-14;
}
		
template<typename GEOSHAPE, UInt DIM> virus2d<GEOSHAPE, DIM>::virus2d(mesh2d<GEOSHAPE>  *_meshPointer)
{
	// chiamo il costruttore di connect
	connect2d<GEOSHAPE>::connect2d(_meshPointer);
	
	// creo la connettività che mi serve 
	connect2d<GEOSHAPE>::buildElementToElement();
	
	// setto la tolleranza
	toll = 1e-14;
}

template<typename GEOSHAPE, UInt DIM> void virus2d<GEOSHAPE, DIM>::setMeshPointer(mesh2d<GEOSHAPE>  *_meshPointer)
{		
	// chiamo il setMeshPointer della classe madre
	connect2d<GEOSHAPE>::setMeshPointer(_meshPointer);
...
// Metodi di get/set 
//
template<typename GEOSHAPE, UInt DIM> inline void virus2d<GEOSHAPE, DIM>::setToll(Real _toll)
{
      toll = _toll;
}

template<typename GEOSHAPE, UInt DIM> inline Real virus2d<GEOSHAPE, DIM>::getToll()
{
      return(toll);
}
...
// Metodi per la diffusione della malattia
//
template<typename GEOSHAPE, UInt DIM> UInt virus2d<GEOSHAPE, DIM>::virusDiffusion(UInt idStart)
{      
	// variabili in uso 
	mesh2d<GEOSHAPE>  * meshP = connect2d<GEOSHAPE>::meshPointer;
...
template<typename GEOSHAPE, UInt DIM> void virus2d<GEOSHAPE, DIM>::virusDiffusion(mesh1d<BSHAPE> * barrier, UInt idStart)
{      
	// variabili in uso 
	mesh2d<GEOSHAPE>  * meshP = connect2d<GEOSHAPE>::meshPointer;
...
/*		            int n;
		            char command [150];
		            n = sprintf(command,"passi%d.inp",contatore);
		            file.fileForParaview(command, meshP);
		            contatore++;*/
...
template<typename GEOSHAPE, UInt DIM> void virus2d<GEOSHAPE, DIM>::createSet(mesh1d<BSHAPE> * barrier, 
								             set<geoElement<BSHAPE> > * finder)
{
	// variabili in uso 
	UInt 			               tmpId;
	meshHandler<mesh2d<GEOSHAPE> , DIM>  handler;
	...
	handler.setMeshPointer(connect2d<GEOSHAPE>::meshPointer);
...
template<typename GEOSHAPE, UInt DIM> pair<bool, UInt>  virus2d<GEOSHAPE, DIM>::newStart(vector<bool> * visited)
{
	// variabile in uso 
	pair<bool, UInt>	result;
...
template<typename GEOSHAPE, UInt DIM> void virus2d<GEOSHAPE, DIM>::findAdiacenti(UInt elemId, vector<UInt> * adiacenti)
{
	// faccio un clear e un reserve
	adiacenti->clear();
	adiacenti->reserve(connect2d<GEOSHAPE>::getElementToElementPointer(elemId)->getNumConnected());
  
	// ciclo su tutti i connessi 
	for(UInt i=0; i<connect2d<GEOSHAPE>::getElementToElementPointer(elemId)->getNumConnected(); ++i)
	{
	      adiacenti->push_back(connect2d<GEOSHAPE>::getElementToElementPointer(elemId)->getConnectedId(i));
	}
}

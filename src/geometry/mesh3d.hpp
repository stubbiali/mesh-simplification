...
/*! Classe che rapprensenta una mesh3d geometrica. La mesh3d è rappresentata come un insieme di punti che vengono connessi da
...
template<typename GEOSHAPE> class mesh3d
{
	//
	// Variabili 
	//
	...
	public:
		...
		/*! Costruttore con delle liste 
		    ...
		    \param nV intero per settare la capacità della lista dei nodi 
		    \param nF intero per settare la capacità della lista degli elementi */
		mesh3d(vector<point> _nodes, vector<geoElement<GEOSHAPE> >  _elements);
		
		/*! Costruttore di copia 
		 \param _mesh3d  mesh 3d da copiare*/
		mesh3d(const mesh3d &_mesh3d);
		
		/*! Operatore ugauglianza 
		 \param _mesh3d  mesh 3d da copiare*/
		mesh3d & operator=(const mesh3d &_mesh3d);
...
// Costruttori
//
template<typename GEOSHAPE> mesh3d<GEOSHAPE>::mesh3d()
{
      // setto le variabili
      maxNumNodes       = 0;
      maxNumElements    = 0;
...
template<typename GEOSHAPE> mesh3d<GEOSHAPE>::mesh3d(UInt nV, UInt nF)
{
      // setto le variabli 
      maxNumNodes       = nV;
      maxNumElements    = nF;
...
template<typename GEOSHAPE> mesh3d<GEOSHAPE>::mesh3d(vector<point> _nodes, vector<geoElement<GEOSHAPE> >  _elements)
{
      // setto le variabili
      maxNumNodes       = 0;
      maxNumElements    = 0;
...
template<typename GEOSHAPE> mesh3d<GEOSHAPE>::mesh3d(const mesh3d &_mesh3d)
{
      // setto le variabili
      maxNumNodes       = _mesh3d.maxNumNodes;
      maxNumElements    = _mesh3d.maxNumElements;
      ...
      // copio
      nodes = _mesh3d.nodes;
      elements = _mesh3d.elements;
}

template<typename GEOSHAPE> mesh3d<GEOSHAPE> & mesh3d<GEOSHAPE>::operator=(const mesh3d &_mesh3d)
{
      // setto le variabili
      maxNumNodes       = _mesh3d.maxNumNodes;
      maxNumElements    = _mesh3d.maxNumElements;
...
template<typename GEOSHAPE> void mesh3d<GEOSHAPE>::clear()
{
      // libero le liste
      nodes.clear();
      elements.clear();
...
template<typename GEOSHAPE> inline UInt mesh3d<GEOSHAPE>::getNumNodes() const
{
	return(nodes.size());
}

template<typename GEOSHAPE> inline UInt mesh3d<GEOSHAPE>::getNumElements() const
{
...
template<typename GEOSHAPE> inline vector<point> * mesh3d<GEOSHAPE>::getNodePointer()
{
	return(&nodes);
}

template<typename GEOSHAPE> inline vector<geoElement<GEOSHAPE> > * mesh3d<GEOSHAPE>::getElementPointer()
{
	return(&elements);
}

template<typename GEOSHAPE> inline point * mesh3d<GEOSHAPE>::getNodePointer(UInt nodeId)
{
	assert(nodeId<nodes.size());
	return(&nodes[nodeId]);
}

template<typename GEOSHAPE> inline geoElement<GEOSHAPE> * mesh3d<GEOSHAPE>::getElementPointer(UInt elemId)
{
	assert(elemId<elements.size());
	return(&elements[elemId]);
}

template<typename GEOSHAPE> inline point mesh3d<GEOSHAPE>::getNode(UInt i) const
{
	assert(i<nodes.size());
	return(nodes[i]);
}
		
template<typename GEOSHAPE> inline geoElement<GEOSHAPE> mesh3d<GEOSHAPE>::getElement(UInt i) const
{
	assert(i<elements.size());
	return(elements[i]);
}

template<typename GEOSHAPE> void mesh3d<GEOSHAPE>::getNodeOfElement(UInt i, vector<point> * connessi) const
{
	assert(i<elements.size());
...
template<typename GEOSHAPE> inline void mesh3d<GEOSHAPE>::insertNode(point p)
{
	p.setId(nodes.size());
	nodes.push_back(p);
}

template<typename GEOSHAPE> void mesh3d<GEOSHAPE>::insertNode(vector<point> * _nodes)
{
...
template<typename GEOSHAPE> inline void mesh3d<GEOSHAPE>::insertElement(geoElement<GEOSHAPE> element)
{
	element.setId(elements.size());
	elements.push_back(element);
}

template<typename GEOSHAPE> void mesh3d<GEOSHAPE>::insertElement(vector<geoElement<GEOSHAPE> > * _elements)
{
	// libero la lista 
	elements.clear();
...
template<typename GEOSHAPE> void mesh3d<GEOSHAPE>::insertElement(set<geoElement<GEOSHAPE> > * _elements)
{
	// libero la lista 
	elements.clear();
...
template<typename GEOSHAPE> void mesh3d<GEOSHAPE>::setUpIds()
{
...
template<typename GEOSHAPE> void mesh3d<GEOSHAPE>::setUpGeoId(UInt geo)
{
	for(UInt i=0; i<elements.size(); ++i)     elements[i].setGeoId(geo);
}

template<typename GEOSHAPE> void mesh3d<GEOSHAPE>::setUpGeoId(map<UInt,UInt> oldToNew)
{
	// variabile in uso 
	UInt geo;
...
// metodi che fanno delle semplici analisi 
//
template<typename GEOSHAPE> void mesh3d<GEOSHAPE>::createBBox(point & pMax, point & pMin)
{
...
template<typename GEOSHAPE> void mesh3d<GEOSHAPE>::createBBox(UInt elemId, point & pMax, point & pMin)
{
...
template<typename GEOSHAPE> Real mesh3d<GEOSHAPE>::minH()
{
...
template<typename GEOSHAPE> Real mesh3d<GEOSHAPE>::maxH()
{
      // variabili temporanea
      mesh1d<typename GEOSHAPE::BShape::BShape>		reticolo;
...
// metodo che genera mesh1d
//
template<typename GEOSHAPE> void mesh3d<GEOSHAPE>::createMesh2d(mesh2d<typename GEOSHAPE::BShape> * faces)
{
	// variabili in uso 
...
template<typename GEOSHAPE> void mesh3d<GEOSHAPE>::createMesh1d(mesh1d<typename GEOSHAPE::BShape::BShape> * reticolo)
{
	// variabili in uso 
...
template<typename GEOSHAPE> void mesh3d<GEOSHAPE>::createMesh0d(mesh0d<typename GEOSHAPE::BShape::BShape::BShape> * cloud)
{
      // variabili in uso
      UInt                                                               geoTmp;
      vector<UInt>                                  listaGeoId(nodes.size(), 0);
      vector<geoElement<typename GEOSHAPE::BShape::BShape::BShape> >      lista;
...
// Stampa
//
template<typename GEOSHAPE> void mesh3d<GEOSHAPE>::print()
{
	cout << "Informazioni della Mesh3d" << endl;

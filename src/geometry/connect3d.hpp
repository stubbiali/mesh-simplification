#ifndef CONNECT3D_HPP_
#define CONNECT3D_HPP_

#include <cassert>	
#include <ctime>
#include <iostream>
#include <vector>
#include <set>

#include "../geometry/mesh1d.hpp"
#include "../geometry/mesh2d.hpp"
#include "../geometry/mesh3d.hpp"

namespace geometry{

using namespace std;

/*! Classe che implementa le connessioni per le mesh3d. Restituisce principalmente le seguenti informazioni:
    
    <ol>
    <li> connettivit\'a nodo-nodo;
    <li> connettivit\'a elemento-elemento;
    <li> connettivit\'a nodo-elemento;
    <li> genera la mesh di bordo.
    </ol>
    
    La modifica e la lettura dei grafi contenuti in questa classe viene affidata ai metodi sviluppati in graphItem. */

template<typename GEOSHAPE> class connect3d
{
		// Un typedef per semplificarci la vita!!
		typedef typename GEOSHAPE::BShape BSHAPE;
	
	public:
		/*! Puntatore all'oggetto mesh3d di cui si dorv\'a creare la connettivit\'a */
		mesh3d<GEOSHAPE>           *  meshPointer;
			  
		/*! Vettore che contiene la connettività nodo-nodo*/
		vector<graphItem> 	       nodeToNode;
		
		/*! Vettore che contiene la connettività elemento-elemento*/
		vector<graphItem> 	 elementToElement;
		
		/*! Vettore che contiene la connettività nodo-elemento*/
		vector<graphItem>           nodeToElement;
	//
	// Costruttori
	//
	public:
		/*! Costruttore */
		connect3d();
		
		/*! Costruttore con il puntatore*/
		connect3d(mesh3d<GEOSHAPE>  *_meshPointer);
				
		/*! Metodo che cambia il puntatore alla mesh3d e resetta tutte le variabili */
		void setMeshPointer(mesh3d<GEOSHAPE>  *_meshPointer);
		
		/*! Metodo che restituisce il puntatore alla mesh puntata dalla classe*/
		mesh3d<GEOSHAPE>  * getMeshPointer();
		
		/*! Metodo che pulisce le liste*/
		void clear();
	
	//
	// Metodi che restituiscono i puntatori alle strutture dati:
	// N.B. questi metodi sevono per avere una maggiore flessibilità nell'editing delle liste
	//
	public:
		/*! Puntatore alla lista delle connettività nodo-nodo*/
		vector<graphItem> * getNodeToNodePointer();
		
		/*! Puntatore alla lista delle connettività elemento-elemento*/
		vector<graphItem> * getElementToElementPointer();
		
		/*! Puntatore alla lista delle connettività nodo-elemento*/
		vector<graphItem> * getNodeToElementPointer();
		
		/*! Puntatore alla connettività nodo-nodo del nodo nodeId
		    \param nodeId identificatore del nodo*/
		graphItem * getNodeToNodePointer(UInt nodeId);
		
		/*! Puntatore alla connettività elemento-elemento dell'elemento elemId
		    \param elemId identificatore dell'elemento */
		graphItem * getElementToElementPointer(UInt elemId);
		
		/*! Puntatore alla connettività nodo-elemento del nodo nodeId
		    \param nodeId identificatore del nodo*/
		graphItem * getNodeToElementPointer(UInt nodeId);
	//
	// Metodi che creano le connessioni partendo solo dall'informazioni della mesh3d
	//
	public:
		/*! Connessioni nodo-nodo*/
		void buildNodeToNode();
		
		/*! Connessioni nodo-elemento */
		void buildNodeToElement();
		
		/*! Connessioni elemento-elemento sfrutta SOLO la struttura dati nodeToElement */
		void buildElementToElement();
		
		/*! TUTTE LE CONNESSIONI */
		void buildConnectivity();
		
		/*! crea la mesh di bordo, sfrutta anche la struttura element-element per le superfici con più di un geoId 
		    \param surf puntatore a un'oggetto che conterrà la mesh di bordo 
		    \param surfToVol mappa che associa gli id della mesh di bordo con quella puntata dalla classe
		    \param allBoundary booleano che indica se cercare o meno i bordi dovuti alla presenza di elementi con
		                       differenti geoId */
		void buildBoundaryConnectivity(mesh2d<BSHAPE> * surf, map<UInt, UInt> * surfToVol, bool allBoundary=true);
		
		/*! Settaggio del bordo 
		    \param allBoundary booleano che indica se cercare o meno i bordi dovuti alla presenza di elementi con
		                       differenti geoId */
		void setUpBoundary(bool allBoundary=true);
		
	//
	// Metodo che stampa le connessioni
	//
	public:
		/*! Riempie le liste di connettivit\'a*/
		void print();
};

//-------------------------------------------------------------------------------------------------------
// IMPLEMENTATION
//-------------------------------------------------------------------------------------------------------

//
// Costruttori
//

template<typename GEOSHAPE> connect3d<GEOSHAPE>::connect3d()
{
	meshPointer    = NULL;
}
		
template<typename GEOSHAPE> connect3d<GEOSHAPE>::connect3d(mesh3d<GEOSHAPE>  *_meshPointer)
{
	meshPointer    = _meshPointer;
}

template<typename GEOSHAPE> void connect3d<GEOSHAPE>::setMeshPointer(mesh3d<GEOSHAPE>  *_meshPointer)
{
	clear();
	meshPointer    = _meshPointer;
}

template<typename GEOSHAPE> mesh3d<GEOSHAPE>  * connect3d<GEOSHAPE>::getMeshPointer()
{
	return(meshPointer);
}
		
template<typename GEOSHAPE> void connect3d<GEOSHAPE>::clear()
{
	meshPointer     = NULL;
	nodeToNode.clear();
	elementToElement.clear();
	nodeToElement.clear();
}

//
// Metodi che restituiscono i puntatori alle strutture dati:
//
template<typename GEOSHAPE> vector<graphItem> * connect3d<GEOSHAPE>::getNodeToNodePointer()
{
	return(&nodeToNode);
}
		
template<typename GEOSHAPE> vector<graphItem> * connect3d<GEOSHAPE>::getElementToElementPointer()
{
	return(&elementToElement);
}
		
template<typename GEOSHAPE> vector<graphItem> * connect3d<GEOSHAPE>::getNodeToElementPointer()
{
	return(&nodeToElement);
}

template<typename GEOSHAPE> graphItem * connect3d<GEOSHAPE>::getNodeToNodePointer(UInt nodeId)
{
	assert(nodeId<meshPointer->getNumNodes());
	return(&nodeToNode[nodeId]);
}
		
template<typename GEOSHAPE> graphItem * connect3d<GEOSHAPE>::getElementToElementPointer(UInt elemId)
{
	assert(elemId<meshPointer->getNumElements());
	return(&elementToElement[elemId]);
}
		
template<typename GEOSHAPE> graphItem * connect3d<GEOSHAPE>::getNodeToElementPointer(UInt nodeId)
{
	assert(nodeId<meshPointer->getNumNodes());
	return(&nodeToElement[nodeId]);
}

//
// Metodi che creano le connessioni partendo solo dall'informazioni della mesh3d 
//
template<typename GEOSHAPE> void connect3d<GEOSHAPE>::buildNodeToNode()
{
	  // Libero la lista
	  nodeToNode.clear();
	  nodeToNode.resize(meshPointer->getNumNodes());
	  
	  // Variabli temporanee
	  UInt 					       id1,id2;
	  Real						   dif;
	  time_t                                     start,end;
	  
	  // output
	  cout << "Costruzione connettività 3d nodoNodo..." << endl;
	  time(&start);
	  
	  
	  // Mi ricavo le informazioni sull'elemento preso in esame per avere una migliore descrizione delle 
	  // sue connettività
	  UInt num = GEOSHAPE::numEdges;
	  
	  // Ciclo sugli elementi
	  for(UInt i=0; i<meshPointer->getNumElements(); ++i)
	  {
		  // metto a posto le connettività "in avanti"
		  for(UInt j=0; j<num*2; j=j+2)
		  {
		      // prendo i nodi 
		      id1 = meshPointer->getElement(i).getConnectedId(GEOSHAPE::edgeConn[j]);
		      id2 = meshPointer->getElement(i).getConnectedId(GEOSHAPE::edgeConn[j+1]);
		      
		      // setto tutto 
		      nodeToNode[id1].add(id2);
		      nodeToNode[id2].add(id1);
		  }
	  }
	  
	  time(&end);
	  dif = difftime(end,start);
	  cout << "Connettività 3d nodoNodo completata: " <<  dif << " sec." << endl;
}
		
template<typename GEOSHAPE> void connect3d<GEOSHAPE>::buildNodeToElement()
{
	  // Libero la lista
	  nodeToElement.clear();
	  nodeToElement.resize(meshPointer->getNumNodes());
	  
	  // Mi ricavo le informazioni sull'elemento preso in esame per avere una migliore descrizione delle 
	  // sue connettività
	  UInt num = GEOSHAPE::numVertices;
	  Real						   dif;
	  time_t                                     start,end;
	  
	  // output
	  cout << "Costruzione connettività 3d nodoElemento..." << endl;
	  time(&start);
	  
	  // Ciclo sugli elementi
	  for(UInt i=0; i<meshPointer->getNumElements(); ++i)
		  for(UInt j=0; j<num; ++j)	
			  nodeToElement[meshPointer->getElement(i).getConnectedId(j)].connectedPushBack(i);
		  
	  time(&end);
	  dif = difftime(end,start);
	  cout << "Connettività 3d nodoElemento completata: " <<  dif << " sec." << endl;
}

template<typename GEOSHAPE> void connect3d<GEOSHAPE>::buildElementToElement()
{
	  // Libero la lista
	  elementToElement.clear();
	  elementToElement.resize(meshPointer->getNumElements());
	  
	  // Sfrutto la lista nodeToElement
	  if(nodeToElement.empty())	  buildNodeToElement();
	  
	  Real						   dif;
	  time_t                                     start,end;
	  
	  // output
	  cout << "Costruzione connettività 3d elementoElemento..." << endl;
	  time(&start);
	  
	  // Variabile temporanea
	  UInt                 elemId;
	  graphItem            comuni;
	  vector<UInt>	     conn,ids;
	  vector<UInt>::iterator   it;
	  
	  // Mi ricavo le informazioni sull'elemento preso in esame per avere una migliore descrizione delle 
	  // sue connettività
	  UInt num     = GEOSHAPE::numFaces;
	  UInt numNode = GEOSHAPE::BShape::numVertices;
	  
	  // Ciclo sugli elementi
	  for(UInt i=0; i<meshPointer->getNumElements(); ++i)
	  {
		  // metto a posto le connettività 
		  for(UInt j=0; j<num*numNode; j=j+numNode)
		  {
		      // resetto gli id della faccia 
		      ids.assign(numNode, 0);
		      
		      // li metto tutti in ids
		      for(UInt s=0; s<numNode; ++s)	ids[s]=meshPointer->getElement(i).getConnectedId(GEOSHAPE::faceConn[j+s]);
			
		      // faccio un grafo con i comuni ai primi due 
		      nodeToElement[ids[0]].common(nodeToElement[ids[1]], &conn);
		      
		      // inzializzo il grafo che conterrà tutto 
		      comuni.setConnectedId(&conn);
		      
		      // faccio tutte le intersezioni
		      for(UInt s=2; s<ids.size(); ++s)
		      {
			    // trovo i comuni 
			    comuni.common(nodeToElement[ids[s]], &conn);
			    
			    // ricostruisco il grafo 
			    comuni.setConnectedId(&conn);
		      }
		      
		      // li metto in oridne perché se no il ciclo di dopo non va bene
		      sort(conn.begin(), conn.end());
		      
		      // a questo livello in conn ci sono tutti gli elementi in comune con tutti i nodi 
		      for(UInt s=0; s<conn.size(); ++s)
		      {
			    // prendo il primo id
			    elemId = conn[s];
			    
			    // lo cerco 
			    it = find(conn.begin(), conn.end(), elemId);
			    
			    // lo devo trovare 
			    assert(it!=conn.end());
			    
			    // elimino dalla lista
			    conn.erase(it);
			    
			    // li metto 
			    elementToElement[elemId].add(conn);
			  
			    // lo reinserisco 
			    conn.push_back(elemId);
			    
			    // li metto in oridne
			    sort(conn.begin(), conn.end());
		      }
		  }
	  }
	  
	  time(&end);
	  dif = difftime(end,start);
	  cout << "Connettività 3d elementoElemento completata: " <<  dif << " sec." << endl;
}
		
template<typename GEOSHAPE> void connect3d<GEOSHAPE>::buildConnectivity()
{
	// Chiama gli altri metodi
	buildNodeToNode();
	buildNodeToElement();
	buildElementToElement();
}

template<typename GEOSHAPE> void connect3d<GEOSHAPE>::buildBoundaryConnectivity(mesh2d<BSHAPE> * surf, 
										map<UInt, UInt> * surfToVol, 
										bool allBoundary)
{
	// variabili in uso
	UInt                            cont;
	vector<UInt>		    conn,ids;
	graphItem			 tmp;
	set<UInt>                         id;
	mesh2d<BSHAPE>                 bordo;
	vector<geoElement<BSHAPE> >    lista;
	vector<bool>		      active;
	map<UInt,UInt>             volToSurf;
	Real				 dif;
	time_t                     start,end;
	  
	// output
	cout << "Costruzione mesh di Bordo in connect3d..." << endl;
	time(&start);
	
	// pulisco bor
	surf->clear();
	surfToVol->clear();
	
	// creo lo scheletro
	meshPointer->createMesh2d(&bordo);
	
	// faccio un reserve
	lista.reserve(bordo.getNumElements());
	
	// setto ids
	ids.resize(BSHAPE::numVertices);
	
	// faccio un resize
	active.assign(bordo.getNumNodes(), false);
	
	// controllo che sia stata creata la connessione nodoElemento
	if(nodeToElement.empty())	buildNodeToElement();
	
	// cilco sugli elementi di bordo per cercare quelli che condividono più di un elemento
	for(UInt i=0; i<bordo.getNumElements(); ++i)
	{
		// metto tutti i nodi in ids
		for(UInt j=0; j<ids.size(); ++j)	ids[j] = bordo.getElement(i).getConnectedId(j);
		  
		// trovo chi hanno in comune
		nodeToElement[ids[0]].common(nodeToElement[ids[1]], &conn);
		
		// metto tutto in tmp
		tmp.setConnectedId(&conn);
		
		// passo agli altri 
		for(UInt j=2; j<ids.size(); ++j)
		{
		      // trovo chi hanno in comune
		      nodeToElement[ids[j]].common(tmp, &conn);
		      
		      // metto tutto in tmp
		      tmp.setConnectedId(&conn);
		}
		
		// se hanno solo un elemento in comune
		if(tmp.getNumConnected()==1)
		{
		    // setto active
		    for(UInt j=0; j<ids.size(); ++j) active[ids[j]] = true;
		
		    lista.push_back(bordo.getElement(i));
		}
		else if(allBoundary)
		{
		    // prendo tutti gli id
		    id.clear();
		    for(UInt j=0; j<tmp.getNumConnected(); ++j)	
			  id.insert(meshPointer->getElement(tmp.getConnectedId(j)).getGeoId());
			
		    // se ho trovato più elementi metto tutto nella lista
		    if(id.size()>1)
		    {
			// setto active
			for(UInt j=0; j<ids.size(); ++j) active[ids[j]] = true;
			
			lista.push_back(bordo.getElement(i));
		    }
		}
	}
	
	// faccio un reserve
	surf->getNodePointer()->reserve(active.size());
	
	// creo la mappa 
	cont = 0;
	for(UInt i=0; i<active.size(); ++i)	
	{
	      if(active[i])
	      {
		  // se il nodo è attivo creo la mappa 
		  surfToVol->insert(pair<UInt,UInt>(cont, i));
		  volToSurf.insert(pair<UInt,UInt>(i, cont));
	      
		  // lo inserisco 
		  surf->insertNode(meshPointer->getNode(i));
	      
		  // incremento cont
		  ++cont;
	      }
	}
	
	// ciclo sulla lista per cambiare gli id
	for(UInt i=0; i<lista.size(); ++i)
	{
	      // metto tutti i nodi in ids
	      for(UInt j=0; j<ids.size(); ++j)		ids[j] = lista[i].getConnectedId(j);
	      
	      // metto tutti i nodi in ids
	      for(UInt j=0; j<ids.size(); ++j)		lista[i].setConnectedId(j, volToSurf[ids[j]]);
	}
	
	// la metto in bor
	surf->insertElement(&lista);
	
	// faccio un setup
	surf->setUpIds();
	
	time(&end);
	dif = difftime(end,start);
	cout << "Costruzione mesh di bordo completata: " <<  dif << " sec." << endl;
}

template<typename GEOSHAPE> void connect3d<GEOSHAPE>::setUpBoundary(bool allBoundary)
{
	// variabli in uso 
	mesh2d<typename GEOSHAPE::BShape>    bor;
	map<UInt, UInt>		      borToCloud;
	
	// creo la mesh di bordo
 	buildBoundaryConnectivity(&bor, &borToCloud);
	
	// setto a tutte le variabili boundary 
	for(UInt i=0; i<meshPointer->getNumNodes(); ++i)	meshPointer->getNodePointer(i)->setBoundary(0);
	
	// inserisco le informazioni di bordo 
	for(UInt i=0; i<bor.getNumNodes(); ++i) meshPointer->getNodePointer(borToCloud[i]).setBoundary(1);
}

//
// Metodo che stampa le connessioni

template<typename GEOSHAPE> void connect3d<GEOSHAPE>::print()
{
	cout << "Lista delle connessioni nodo-nodo" << endl;
	for(UInt i=0; i<meshPointer->getNumNodes(); ++i)
	{
	      cout << "Node " << meshPointer->nodes[i].getId() << ": ";
	      for(UInt j=0; j<nodeToNode[i].getNumConnected(); ++j)
	      {
		    cout << nodeToNode[i].getConnectedId(j) << "  ";
	      }
	      cout << endl;
	}
	cout << endl;
	
	cout << "Lista delle connessioni elemento-elemento" << endl;
	for(UInt i=0; i<meshPointer->getNumElements(); ++i)
	{
	      cout << "Element " << meshPointer->elements[i].getId() << ": ";
	      for(UInt j=0; j<elementToElement[i].getNumConnected(); ++j)
	      {
		    cout << elementToElement[i].getConnectedId(j) << "  ";
	      }
	      cout << endl;
	}
	cout << endl;
	
	cout << "Lista delle connessioni nodo-elemento" << endl;
	for(UInt i=0; i<meshPointer->getNumNodes(); ++i)
	{
	      cout << "Node " << meshPointer->nodes[i].getId() << ": ";
	      for(UInt j=0; j<nodeToElement[i].getNumConnected(); ++j)
	      {
		    cout << nodeToElement[i].getConnectedId(j) << "  ";
	      }
	      cout << endl;
	}
	cout << endl;
}



}

#endif

